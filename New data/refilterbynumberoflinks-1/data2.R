...
f <- read_csv("..\\global-bilat-flow-sex\\est\\bilat_mig_sex.csv")
...
str(m)
# m$`1990`$da_min_closed$female[1:5, 1:5]
...
for(i in 1:6){
  ...
  write(x = j1, file = paste0("./json/flow_female_", x[i],".json"))
  
  k1 <- toJSON(k0, auto_unbox = TRUE, pretty = TRUE)
  write(x = k1, file = paste0("./json/flow_male_", x[i],".json"))
}
