...
f <- read_csv("..\\data-unpd/tims/data/bilat/tims2020.csv", guess_max = 1e6)
...
d0 <- f %>%
  filter(sex == "b",
         pob_code < 900, 
         por_code < 900) %>%
  select(year, pob_code, por_code, stock) %>%
  mutate(
    orig = countrycode(
      sourcevar = pob_code, custom_dict = dict_ims,
      origin = "iso3n", destination = "iso3c"),
    dest = countrycode(
      sourcevar = por_code, custom_dict = dict_ims,
      origin = "iso3n", destination = "iso3c"),
...
j0 <- list(
  ...
  total_inflow = d3 %>%
    ...
    rename(dest = country) %>%
    select(year, dest, fb) %>%
    xtabs(formula = round(fb) ~ year + dest, data = .,) %>%
    array_tree(),
  total_outflow = d3 %>%
    filter(sex == "b") %>%
    rename(orig = country) %>%
    select(year, orig, dispora) %>%
    xtabs(formula = round(dispora) ~ year + orig, data = .,) %>%
    array_tree()
)
str(j0, max.level = 2)
