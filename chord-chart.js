...
function drawChords(chordData, commonData, specificRawData, metadataCsv, config, chartWidth, chartHeight) {
    ...
    function computedChords(data, useMaxScale) {
        ...
        if (useMaxScale) {
            // The chord() function returns chords and attaches the groups.
            // The groups are sorted by value.
            const sortedGroups = currentYearSortedLayout.groups;

            // Create a lookup map from the sorted groups, keyed by the original, stable index.
            // This provides the correct group value and angles for the current year's sorted layout.
            const currentYearGroupsByIndex = sortedGroups.reduce((acc, g) => {
                acc[g.index] = g;
                return acc;
            }, {});
